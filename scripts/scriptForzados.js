...
var ipL, ipG, ipM, ipA, ipAm, ipMasb, radMas, ipFuer; // Entradas de longitud, gravedad, masa y amplitud
...
function start() {
  ...
  radMas = getElement('radMas'); //Obtener la entrada del radio de la masa
  ipA = getElement("ipAb"); // Obtener la entrada de amplitud
  ipAm = getElement("ipAm"); // Obtener la entrada de amortiguacion
  ipFuer = getElement("ipFuer"); // Obtener la entrada de fuerza
  ...
  m = 1; // Masa inicial
  r = 2; // radio inicial
  amortiguacion = 0; //amortiguacion inicial
  ...
  ipFuer.onkeydown = reactionEnter; // Asignar evento de tecla presionada para la entrada de fuerza
  ...
  ipFuer.onblur = reaction; // Asignar evento de perdida de foco para la entrada de fuerza
...
function input() {
  ...
  alpha0 = inputNumber(ipA, 2, true, 0.1, 90) * deg; // Obtener y procesar la entrada de amplitud en radianes
  amortiguacion = inputNumber(ipAm, 2, true, 0, 50); // Obtener y procesar la entrada de amortiguacion
  fuerzaN = inputNumber(ipFuer, 2, true, 0, 50); // Obtener y procesar la entrada de fuerza
...
function updateInput() {
  ...
  ipFuer.value = ToString(fuerzaN, 2, true); //Establecer el valor de la entrada de fuerza
...
function drawElongation() {
  ...
  writeValue("Elongación", s, "m", 3, xD, xD + 200, height - 70);
  writeValue("(" + "Máximo", sMax, "m" + ")", 3, xD, xD + 200, height - 50);
}
...
function drawVelocity() {
  ...
  writeValue("Velocidad", v, "m/s", 3, xD, xD + 200, height - 70);
  writeValue("(" + "Máximo", vMax, "m/s" + ")", 3, xD, xD + 200, height - 50);
}
...
function drawAcceleration() {
  ...
  var mps2 = "m/s²";
  writeValue("Aceleración (Componente Tangencial)", a, mps2, 3, xD - 30, xD + 220, height - 70);
  writeValue("(" + "Máximo", aMax, mps2 + ")", 3, xD - 30, xD + 220, height - 50);
}
...
function drawForce() {
  ...
  writeValue("Fuerza (Componente Tangencial)", f, "N", 3, xD - 30, xD + 220, height - 70);
  writeValue("(" + "Máximo", fMax, "N" + ")", 3, xD - 30, xD + 220, height - 50);
}
...
function drawEnergy() {
  ...
  ctx.fillStyle = colorElongation;
  writeValue("Energía Potencial", eP, "J", 3, xD, xD + 200, height - 90);
  ctx.fillStyle = colorVelocity;
  writeValue("Energía Cinética", eK, "J", 3, xD, xD + 200, height - 70);
  ctx.fillStyle = "#000000";
  writeValue("Energía Total", e, "J", 3, xD, xD + 200, height - 50);
...
function paint() {
  ...
  alignText(sOmega, 1, ax, height - 70); // Coloca el texto debajo del período
  // Dibuja el transportador
  drawTransportador(ax, ay, 150);

  var amplitud = "Valor de amplitud:" + alpha;
  amplitud = amplitud.replace(".", decimalSeparator);
  alignText(amplitud, 1, ax, height - 50); // Coloca el texto debajo del período
}
